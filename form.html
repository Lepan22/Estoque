...
<body>
  ...
  <script type="module">
        ...
        .then(snapshot => {
          ...
          if (currentItens.length === 0) {
            ...
            currentItens.forEach((item, idx) => {
              ...
              // Puxamos todos os campos, com fallback
          ...
          // Se já finalizado, desabilita tudo
    ...
    // Finaliza evento e salva alterações
    btnFinal.addEventListener("click", () => {
      btnFinal.disabled    = true;
      btnFinal.textContent = "Processando...";

      // Rebusca original para manter índice
      get(refEvento).then(snapshot => {
        const ev        = snapshot.val();
        const itensOrig = Array.isArray(ev.itens) ? ev.itens : [];
        const novos     = [];

        document.querySelectorAll("#itens-tabela tbody tr[data-item-index]")
          .forEach(tr => {
